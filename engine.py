...
# Tier 3 adjudication
# ===========================================================================
def ai_diagnose(facts: dict):
    """
    Given engine-computed facts about a variance, return (reason_code, text).

    The offline path is deterministic so the engine can be developed and
    scored without network access or spend. Swapping in a real Claude/Gemini
    call means replacing the body below -- the contract does not change, and
    the model is still never asked to compute anything.
    """
    if USE_LLM:
...
def _llm_diagnose(facts: dict):
    """Real model call. Kept isolated so the offline path stays pure."""
    raise NotImplementedError(
        "Wire the Claude/Gemini client here. Send `facts` as-is, require the "
        "model to return one of REASON_LEGEND's codes plus one sentence, and "
        "reject any response containing a number the engine did not supply.")
...
def reconcile(orders: pd.DataFrame, setls: pd.DataFrame) -> pd.DataFrame:
    ...
    def emit(rid, tier, status, reason, fee_type, expected, received, delta,
             at_risk, explanation, matched, age):
        out.append(dict(
    ...
    for o in orders.to_dict("records"):
        ...
        if delta == 0:
            ...
            tier = path_tier or 3
            emit(rid, tier, "MANUAL_REVIEW", reason, "", amt, recv, delta,
                 abs(delta), why, m["settlement_id"], age)
